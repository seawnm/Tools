...
select ServerName,Login,SPID,HostName,DBName,EventInfo,Status,CPUTime,DiskIO,LastBatch
from (
...
FROM        #sp_who2 a
left join #inputbuffer b
on a.SPID=b.SPID
) aa
where SPID>50 and Status='RUNNABLE                      '
group by ServerName,Login,SPID,HostName,DBName,EventInfo,Status,CPUTime,DiskIO,LastBatch
order by CPUTime desc,DiskIO desc,ServerName,DBName,Login,HostName


/*
select ServerName,DBName,Login,HostName,max(Update_time) Update_time
from S8200FTPWTS01.HQDB.dbo.DBSource_Record 
where ServerName='S8200FTPWTS01'
group by ServerName,Login,HostName,DBName
order by ServerName,DBName,Login,HostName
